  ...
  <div>
    ...
    <div class="flex flex-col justify-between items-stretch pages">
      <div class="lg:row-span-3 relative flex-1" ref="captureTarget">
        ...
        <div class="blur-background flex items-center justify-center">
          <img
            ...
            alt="unicorn"
            class="p-4 w-48 h-48 object-contain"
          />
        </div>
      ...
      <div class="flex justify-between items-center">
        <button
          @click="prevPage"
          class="p-4 m-4 click-btn rounded-full bg-transparent"
        >
          <img src="../assets/svg/left.svg" />
        </button>
        <div class="steps rounded-full">
          <div
            class="flex justify-between backdrop-blur-xl items-center h-full rounded-full gap-1"
          >
            <template v-for="(color, index) in rainbowColor">
              <button
                class="step-btn transition-all duration-300"
                v-if="index < rainbowColor.length - 1"
                :class="currentPage === index + 1 ? 'flex-1' : ''"
                :style="{
                  'background-image': `linear-gradient(to right, ${color}, ${
                    currentPage === index + 1 ? rainbowColor[index + 1] : color
                  })`,
                }"
              ></button>
            </template>
        ...
        <button
          @click="nextPage"
          class="p-4 m-4 rounded-full click-btn bg-transparent"
        >
...
const nextPage = () =>
  currentPage.value < 6 ? (currentPage.value += 1) : null;
